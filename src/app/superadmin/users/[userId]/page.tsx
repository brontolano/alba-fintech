...
export default async function EditUserPage({ params, searchParams }: Props) {
  ...
  const userIdNum = isNew ? null : parseInt(userId);

  const [tenancy, user] = await Promise.all([
    prisma.tenant.findMany({ where: { isActive: true }, select: { id: true, name: true, appName: true, units: { select: { id: true, name: true, type: true, retailEnabled: true } } } }),
    isNew ? null : prisma.user.findUnique({ where: { id: userIdNum! } }),
  ]);

  if (!isNew && !user) redirect("/superadmin/managers");
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            {isNew ? "Akun Baru" : `Edit ${user?.name}`}
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            {isNew ? "Buat akun pengguna baru" : "Perbarui informasi akun"}
          </p>
        </div>
