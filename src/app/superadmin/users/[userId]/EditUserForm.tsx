...
import { Save, X, Loader2, AlertCircle, Trash2 } from "lucide-react";
...
export default function EditUserForm({ user, tenants, unitsByTenant }: Props) {
  const [role, setRole] = useState<Exclude<Props["user"]["role"], "Superadmin">>(
    user.role as "Pimpinan" | "Manager" | "Staff"
  );
  const [tenantId, setTenantId] = useState<number>(user.tenantId);
  const action = (fd: FormData) =>
    updateUserAction(user.id, { ok: false }, fd);
  const [state, formAction, isPending] = useFormState(action, { ok: false });

  const availableUnits = useMemo(() => unitsByTenant[tenantId] || [], [tenantId, unitsByTenant]);
      ...
      <div className="bg-surface-container-lowest rounded-xl-custom border border-outline-variant p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={inputClass}
            />
            {errorFor("email") && <p className="text-error mt-1 font-caption">{errorFor("email")}</p>}
        </div>
      </div>
        ...
        <div>
          ...
          <input
            ...
            className={inputClass}
          />
          {errorFor("password") && <p className="text-error mt-1 font-caption">{errorFor("password")}</p>}
        ...
        <div>
          ...
          <select
            ...
            value={role}
            onChange={(e) =>
              setRole(e.target.value as Exclude<Props["user"]["role"], "Superadmin">)
            }
            disabled={isPending}
            className={inputClass}
          >
          ...
          {errorFor("role") && <p className="text-error mt-1 font-caption">{errorFor("role")}</p>}
       </div>
    </div>
      ...
      <div className="bg-surface-container-lowest rounded-xl-custom border border-outline-variant p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {errorFor("tenantId") && <p className="text-error mt-1 font-caption">{errorFor("tenantId")}</p>}
        </div>
          ...
          {showUnitSelector && (
            <div>
              ...
              {errorFor("unitId") && <p className="text-error mt-1 font-caption">{errorFor("unitId")}</p>}
              {availableUnits.length === 0 && (
                <p className="font-caption text-caption text-warning mt-1">
                  Tenant ini belum memiliki unit. Buat unit terlebih dahulu.
            </p>
              )}
          </div>
          )}
